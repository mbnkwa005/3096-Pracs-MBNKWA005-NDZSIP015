...
int size = sizeof(patterns)/sizeof(patterns[0]); // size of patterns array

uint16_t cAddress = 0; // current Address
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  {
		  __HAL_TIM_SET_AUTORELOAD(&htim16, ARR/2);
	  }
...
/* USER CODE END MX_GPIO_Init_1 */
  ...
  /* Set several pins to LOW on dedicated GPIO Pin
   ...
   * LL_SYSCFG_SetEXTISource(uint32_t Port, uint32_t Line)
   * */
...
void TIM16_IRQHandler(void)
{
	...
	uint8_t rfAddress = read_from_address(cAddress); // rfAddress - read_from_address return value

	if (rfAddress != patterns[cAddress]){

		/* Writes output data register for the port.
		 * LL_GPIO_WriteOutputPort(GPIO_TypeDef* GPIOx, uint32_t PortValue)
		 * */
		LL_GPIO_WriteOutputPort(LED0_GPIO_Port, 0b00000001);
	}
	else{
		LL_GPIO_WriteOutputPort(LED0_GPIO_Port, rfAddress);
	}
	// Cycle to the next binary value.
	cAddress = (cAddress + 1) % size;
}
